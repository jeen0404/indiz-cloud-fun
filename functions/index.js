...
/* if(caption.includes('#')){
  var HashTags = caption.match(/(^|\s)#(\w+)/g).map(function(v){return v.trim().substring(1);});
  var db       = admin.database().ref();
  var isHashTagNameAvailable;
  HashTags.forEach(async (element) => {
    //var TggedUserDetails =  await getFirestore().collection('users').where("username","==",element).get();
    db.once("HashTag").then(function(snapshot){
      const isDBRefrenceAvailable = snapshot.exists(); 
      if(isDBRefrenceAvailable ) { isHashTagNameAvailable = snapshot.hasChild(element); }
    });

    if(isHashTagNameAvailable) {
      //increase the count of HashTag only then put the post in Respective Hash Categories Feed

    }else{
      //Insert this New HashTag in the DB, set count to one and put the place in the respective Categories Feed. 
      db.set({
        HashTagName : element,
        userId : postData.data().userid
      }); 
    }
    getFirestore().collection("HashTags").doc("HashTagNames").set({
      HastTagName : element,
      userId : postData.data().userid,
      postId : context.params.postId,
    });
